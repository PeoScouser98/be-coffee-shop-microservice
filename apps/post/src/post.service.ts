...
export class PostService {
	...
	public async createPost(payload: PostDTO) {
		const createdPost = await this.postRepository.createOne(payload)
		if (!createdPost)
	...
	public async updatePostById(
		...
		const updatedPost = await this.postRepository.updateOneById(postId, payload)
		if (!updatedPost)
			return new ServiceResult(null, {
	...
	public async publishPost(postId: string) {
		const publishedPost = await this.postRepository.updateOneById(postId, {
			is_published: true,
			is_draft: false,
	...
	public async getAllPosts(filterQuery?: FilterQuery<PostDocument>) {
		const posts = await this.postRepository.findOneWithFilter({ ...filterQuery })
	...
	public async getPostBySlug(slug: string) {
		const post = await this.postRepository.findOneWithFilter({ slug })
		if (!post)
			return new ServiceResult(null, {
				message: this.i18nService.t('error_messages.post.not_found'),
				errorCode: HttpStatus.NOT_FOUND
